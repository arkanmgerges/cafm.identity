...
class AuthzAppServiceListener(AuthzAppServiceServicer):
    ...
    def isAllowedByToken(self, request, context):
        try:
            ...
            logger.debug(
                f'[{AuthzAppServiceListener.isAllowedByToken.__qualname__}] - receive request with token: {request.token}, data: {request.data}')
            authzAppService: AuthorizationApplicationService = AppDi.instance.get(AuthorizationApplicationService)
            isAllowed: bool = authzAppService.isAllowedByToken(token=request.token, data=request.data)
            logger.debug(
                f'[{AuthzAppServiceListener.isAllowedByToken.__qualname__}] - returned isAllowed: {isAllowed}')
            return AuthzAppService_isAllowedResponse(isAllowed=isAllowed)
        except Exception as e:
            logger.warn(
                f'[{AuthzAppServiceListener.isAllowedByToken.__qualname__}] - exception, Unknown: {e}')
            raise e
