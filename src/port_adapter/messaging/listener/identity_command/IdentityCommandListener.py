...
class IdentityCommandListener:
    ...
    def run(self):
        consumer: Consumer = AppDi.Builder.buildConsumer(
            groupId=os.getenv("CAFM_IDENTITY_CONSUMER_GROUP_IDENTITY_CMD_NAME", ""),
            autoCommit=False,
            partitionEof=True,
        ...
        try:
            while True:
                ...
                if msg.error():
                    if msg.error().code() == KafkaError._PARTITION_EOF:
                        logger.info(
                            f"[{IdentityCommandListener.run.__qualname__}] msg reached partition eof: {msg.error()}"
                        )
                    else:
                    ...
                    while not isMsgProcessed:
                        ...
                        try:
                            msgData = msg.value()
                            logger.debug(
                                f"[{IdentityCommandListener.run.__qualname__}] received message data = {msgData}"
                            )
                            ...
                            logger.debug(
                                f"[{IdentityCommandListener.run.__qualname__}] cleanup event publisher"
                            )
            ...
            logger.info(
                f"[{IdentityCommandListener.run.__qualname__}] Shutting down the process"
            )
        finally:
