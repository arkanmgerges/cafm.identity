...
class RevokeAssignmentRoleToPermissionHandler(Handler):
    ...
    def __init__(self):
        self._commandConstant = CommonCommandConstant.REVOKE_ASSIGNMENT_ROLE_TO_PERMISSION
    ...
    def handleCommand(self, name: str, data: str, metadata: str) -> dict:
        logger.debug(
            f'[{RevokeAssignmentRoleToPermissionHandler.handleCommand.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}')
        ...
        appService.revokeAssignmentRoleToPermission(roleId=dataDict['role_id'],
