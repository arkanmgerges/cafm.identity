...
class RevokeAssignmentRoleToUserGroupHandler(Handler):
    ...
    def canHandle(self, name: str) -> bool:
        return name == IdentityCommandConstant.REVOKE_ASSIGNMENT_ROLE_TO_USER_GROUP.value
    ...
    def handleCommand(self, name: str, data: str, metadata: str) -> dict:
        ...
        appService.revokeAssignmentRoleToUserGroup(roleId=dataDict['role_id'], userGroupId=dataDict['user_group_id'], token=metadataDict['token'])
        return {'name': IdentityCommandConstant.REVOKE_ASSIGNMENT_ROLE_TO_USER_GROUP.value, 'createdOn': round(time.time() * 1000),
