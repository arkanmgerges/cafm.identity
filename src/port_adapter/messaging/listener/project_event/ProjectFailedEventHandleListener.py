    ...
    def run(self):
        self._process(
            consumerGroupId=os.getenv(
                "CAFM_IDENTITY_CONSUMER_GROUP_PROJECT_FAILED_EVT_HANDLE_NAME",
                "cafm.identity.consumer-group.project-failed-evt-handle",
            ),
            consumerTopicList=[
                os.getenv("CAFM_IDENTITY_PROJECT_FAILED_EVENT_HANDLE_TOPIC", "cafm.identity.project-failed-evt-handle")
            ],
        )
