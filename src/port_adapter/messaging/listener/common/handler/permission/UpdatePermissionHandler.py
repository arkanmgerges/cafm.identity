...
class UpdatePermissionHandler(Handler):
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{UpdatePermissionHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        data = copy(dataDict)
        dataDict["id"] = dataDict.pop("permission_id")
        appService.updatePermission(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict["token"])
