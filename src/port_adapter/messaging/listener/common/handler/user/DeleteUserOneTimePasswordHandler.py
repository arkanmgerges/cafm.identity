...
class DeleteUserOneTimePasswordHandler(Handler):
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{DeleteUserOneTimePasswordHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.deleteUserOneTimePassword(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict["token"])
