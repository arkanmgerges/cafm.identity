...
class DeletePermissionContextHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.DELETE_PERMISSION_CONTEXT
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{DeletePermissionContextHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        data = copy(dataDict)
        dataDict["id"] = dataDict.pop("permission_context_id")
        appService.deletePermissionContext(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict["token"])
