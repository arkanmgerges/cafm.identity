...
class RevokeAssignmentResourceToResourceHandler(Handler):
    def __init__(self):
        self._commandConstant = (
            CommonCommandConstant.REVOKE_ASSIGNMENT_RESOURCE_TO_RESOURCE
        )
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{RevokeAssignmentResourceToResourceHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.revokeAssignmentResourceToResource(
