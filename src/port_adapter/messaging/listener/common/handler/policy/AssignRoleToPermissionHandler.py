...
class AssignRoleToPermissionHandler(Handler):
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{AssignRoleToPermissionHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.assignRoleToPermission(**Util.snakeCaseToLowerCameCaseDict(dataDict), token=metadataDict["token"])
