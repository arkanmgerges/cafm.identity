...
class RevokeAssignmentPermissionToPermissionContextHandler(Handler):
    def __init__(self):
        self._commandConstant = CommonCommandConstant.REVOKE_ASSIGNMENT_PERMISSION_TO_PERMISSION_CONTEXT
    ...
    def handleMessage(self, messageData: dict, extraData: dict = None) -> dict:
        ...
        logger.debug(
            f"[{RevokeAssignmentPermissionToPermissionContextHandler.handleMessage.__qualname__}] - received args:\ntype(name): {type(name)}, name: {name}\ntype(data): {type(data)}, data: {data}\ntype(metadata): {type(metadata)}, metadata: {metadata}"
        )
        ...
        appService.revokeAssignmentPermissionToPermissionContext(
