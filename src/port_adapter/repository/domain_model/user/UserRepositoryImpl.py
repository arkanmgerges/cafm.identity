...
class UserRepositoryImpl(UserRepository):
    ...
    def createUser(self, obj: User, tokenData: TokenData):
        ...
        actionFunction = """
            function (params) {                                            
                ...
                if (res.length == 0) {
                    p = params['resource']
                    res = db.resource.insert({_key: p['id'], id: p['id'], email: p['email'], password: p['password'], 
                                              type: p['type']});
                    fromDocId = res['_id'];
    ...
    def updateUser(self, obj: User, tokenData: TokenData) -> None:
        ...
        if repoObj != obj:
            ...
            logger.debug(
                f"[{UserRepositoryImpl.updateUser.__qualname__}] - Update user with id: {obj.id()}"
            )
    ...
    def deleteUserOneTimePassword(self, obj: User, tokenData: TokenData):
        ...
        if oldObject.isPasswordOneTimePassword():
            ...
            logger.debug(
                f"[{UserRepositoryImpl.deleteUserOneTimePassword.__qualname__}] - Delete user password with user id: {obj.id()}"
            )
            bindVars = {"id": obj.id()}
            queryResult: AQLQuery = self._db.AQLQuery(
                aql, bindVars=bindVars, rawResults=True
    ...
    def setUserPassword(self, obj: User, tokenData: TokenData):
        ...
        logger.debug(
            f"[{UserRepositoryImpl.setUserPassword.__qualname__}] - Set user password with user id: {obj.id()}"
        )
    ...
    def userByEmail(self, email: str) -> User:
        logger.debug(
            f"[{UserRepositoryImpl.userByEmail.__qualname__}] - with email = {email}"
        )
        aql = """
            FOR d IN resource
    ...
    def userByEmailAndPassword(self, email: str, password: str) -> User:
        logger.debug(
            f"[{UserRepositoryImpl.userByEmailAndPassword.__qualname__}] - with email = {email}"
        )
        aql = """
            FOR d IN resource
        ...
        if len(result) == 0:
            logger.debug(
                f"[{UserRepositoryImpl.userByEmailAndPassword.__qualname__}] email: {email}"
            )
            raise UserDoesNotExistException(email)
    ...
    def users(
        ...
        result = self._policyService.resourcesOfTypeByTokenData(
            PermissionContextConstant.USER.value,
            tokenData,
