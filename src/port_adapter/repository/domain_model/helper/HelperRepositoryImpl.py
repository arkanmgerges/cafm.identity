...
class HelperRepositoryImpl(HelperRepository):
    ...
    def roleDocumentId(self, id: str):
        ...
        if len(result) == 0:
            logger.debug(f'[{HelperRepositoryImpl.roleDocumentId.__qualname__}] role id: {id}')
    ...
    def userDocumentId(self, id: str):
        ...
        result = queryResult.result
        if len(result) == 0:
            logger.debug(f'[{HelperRepositoryImpl.userDocumentId.__qualname__}] user id: {id}')
    ...
    def resourceDocumentId(self, resource: Resource):
        ...
        result = queryResult.result
        if len(result) == 0:
            logger.debug(f'[{HelperRepositoryImpl.resourceDocumentId.__qualname__}] resource id: {resource.id()}')
            raise RoleDoesNotExistException(
