...
class UserGroupRepositoryImpl(UserGroupRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{UserGroupRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def userGroupByName(self, name: str) -> UserGroup:
        ...
        if len(result) == 0:
            logger.debug(
                f"[{UserGroupRepositoryImpl.userGroupByName.__qualname__}] {name}"
            )
    ...
    def userGroupById(self, id: str) -> UserGroup:
        ...
        if len(result) == 0:
            logger.debug(
                f"[{UserGroupRepositoryImpl.userGroupById.__qualname__}] user group id: {id}"
            )
    ...
    def userGroups(
        ...
        result = self._policyService.resourcesOfTypeByTokenData(
            PermissionContextConstant.USER_GROUP.value,
            tokenData,
        ...
        return {
            "items": [UserGroup.createFrom(id=x["id"], name=x["name"]) for x in items],
