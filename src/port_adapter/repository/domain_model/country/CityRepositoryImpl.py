...
class CityRepositoryImpl(CityRepository):
    def __init__(self):
        try:
            ...
            logger.warn(
                f"[{CityRepositoryImpl.__init__.__qualname__}] Could not connect to the db, message: {e}"
            )
    ...
    def cityById(self, id: str) -> City:
        ...
        if len(result) == 0:
            logger.debug(
                f"[{CityRepositoryImpl.cityById.__qualname__}] country id: {id}"
            )
            raise CountryDoesNotExistException(f"city id: {id}")
        cityName = result[0]["city_name"] if "city_name" in result[0] else ""
        subdivisionOneIsoCode = (
            result[0]["subdivision_1_iso_code"]
            if "subdivision_1_iso_code" in result[0]
            else ""
        )
        subdivisionOneIsoName = (
            result[0]["subdivision_1_name"] if "subdivision_1_name" in result[0] else ""
        )
        return City.createFrom(
            id=result[0]["geoname_id"],
            localeCode=result[0]["locale_code"],
            continentCode=result[0]["continent_code"],
            continentName=result[0]["continent_name"],
            countryIsoCode=result[0]["country_iso_code"],
