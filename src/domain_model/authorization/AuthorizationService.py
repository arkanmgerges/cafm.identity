...
class AuthorizationService:
    def __init__(
        self, authzRepo: AuthorizationRepository, policyService: PolicyControllerService
    ):
    ...
    def roleAccessPermissionsData(
        ...
        return self._policyService.roleAccessPermissionsData(
            tokenData=tokenData, includeAccessTree=includeAccessTree
        )
    ...
    @debugLogger
    def _verifyActionByPermissionWithPermissionContext(
        self,
        requestedPermissionAction: PermissionAction,
        requestedContextData: ContextDataRequest,
        roleAccessPermissionsData: List[RoleAccessPermissionData],
        ...
        for item in roleAccessPermissionsData:
            ...
            for permissionWithPermissionContexts in permissionsWithPermissionContexts:
                ...
                permissionContexts: List[
                    PermissionContext
                ] = permissionWithPermissionContexts.permissionContexts
                ...
                if requestedPermissionAction.value in permission.allowedActions():
                    ...
                    for permissionContext in permissionContexts:
                        ...
                        if (
                            requestedContextData.dataType
                            == ContextDataRequestConstant.RESOURCE_TYPE
                        ):
                            if self._checkForResourceTypeRequest(
                                requestedPermissionAction,
                                permissionContext,
                                tokenData,
                                item.accessTree,
                                item.ownerOf,
                        ...
                        if (
                            requestedContextData.dataType
                            == ContextDataRequestConstant.PERMISSION
                        ):
                            reqObject: PermissionContextDataRequest = (
                                PermissionContextDataRequest.castFrom(
                                    requestedContextData
                                )
                            )
                            if (
                                reqObject.type == PermissionContextConstant.PERMISSION
                                and permissionContext.type()
    ...
    @debugLogger
    def _checkForResourceTypeRequest(
        self,
        requestedPermissionAction: PermissionAction,
        ...
        resourceTypeContextDataRequest: ResourceTypeContextDataRequest = (
            ResourceTypeContextDataRequest.castFrom(requestedContextData)
        )
        ...
        if permissionContext.type() == PermissionContextConstant.RESOURCE_TYPE.value:
            ...
            if "name" in data:
                ...
                if resourceTypeContextDataRequest.resourceType == data["name"]:
                    ...
                    if requestedPermissionAction in [
                        ...
                        if requestedObject.type == RequestedAuthzObjectEnum.RESOURCE:
                            ...
                            if self._isDeniedInstance(
                                requestedObject.obj, requestedPermissionAction
                            ):
                                return False
                            return self._treeCheck(
                                accessTree,
                                requestedObject.obj,
                                requestedPermissionAction,
                            )
        ...
        for treeItem in accessTree:
            ...
            if nodeContentType is AccessNodeContentTypeConstant.RESOURCE_INSTANCE:
                nodeDataContent: ResourceInstanceAccessNodeContent = (
                    ResourceInstanceAccessNodeContent.castFrom(treeItem.data.content)
                )
                ...
                if len(treeItem.children) > 0:
                    result = self._treeCheck(
                        treeItem.children, requestedResource, requestedPermissionAction
                    )
                    if result is True:
                        return True
            return False
    ...
    @debugLogger
    def _isDeniedInstance(
        self, resource: Resource, requestedPermissionAction: PermissionAction
    ) -> bool:
        id = resource.id()
    ...
    @debugLogger
    def _populateDeniedResources(
        self, roleAccessPermissionsData: List[RoleAccessPermissionData]
    ):
        for item in roleAccessPermissionsData:
            for permissionWithPermissionContexts in item.permissions:
                ...
                for action in permission.deniedActions():
                    for (
                        permContext
                    ) in permissionWithPermissionContexts.permissionContexts:
                        if (
                            permContext.type()
