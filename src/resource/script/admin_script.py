...
def init_geo_data():
    try:
        ...
        click.echo(click.style("Importing cities & countries", fg='green'))
        if None is None:
            with open('src/resource/maxmind/GeoLite2-Country-Locations-en.csv', newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    if row['country_iso_code'] == '':
                        ...
                        cnt += 1
                    id = str(uuid.uuid4())
                    countries.append(dict(id=id, geoNameId=row['geoname_id'], localeCode=row['locale_code'],
        ...
        if None is None:
            with open('src/resource/maxmind/GeoLite2-City-Locations-en.csv', newline='') as csvfile:
                ...
                for row in reader:
                    ...
                    id = str(uuid.uuid4())
                    cities.append(
                        dict(id=id, geoNameId=row['geoname_id'], localeCode=row['locale_code'],
                             continentCode=row['continent_code'],
        ...
        actionFunction = '''
            function (params) {
                queryCountry = `UPSERT {geo_name_id: @geoNameId}
                ...
                queryCity = `UPSERT {geo_name_id: @geoNameId}
                ...
                for (let i = 0; i < params['countries'].length; i++) {
                    ...
                    let p = {'id': country['id'], 'geoNameId': country['geoNameId'], 'localeCode': country['localeCode'],
                    ...
                    db._query(queryCountry, p).execute();
                }
                
                for (let i = 0; i < params['cities'].length; i++) {
                    let city = params['cities'][i];
                    let p = {'id': city['id'], 'geoNameId':city['geoNameId'], 'localeCode':city['localeCode'], 
                    ...
                    'subdivisionOneIsoCode': city['subdivisionOneIsoCode'], 'subdivisionOneIsoName': city['subdivisionOneIsoName'],
                    'subdivisionTwoIsoCode': city['subdivisionTwoIsoCode'], 'subdivisionTwoIsoName': city['subdivisionTwoIsoName'],
                    'cityName': city['cityName'], 'metroCode': city['metroCode'], 'timeZone': city['timeZone'],
                    'isInEuropeanUnion': city['isInEuropeanUnion']};
                    db._query(queryCity, p).execute();
                }
            }
            '''
        ...
        db.transaction(collections={'write': ['country', 'city']}, action=actionFunction, params=params,
                       waitForSync=True)
    except Exception as e:
        click.echo(click.style(str(e), fg='red'))
        exit(0)
...
def createCountry(db, geoNameId, localeCode, continentCode, continentName, countryIsoCode, countryName,
                  isInEuropeanUnion):
    aql = '''
...
def createCity(db, geoNameId, localeCode, continentCode, continentName, countryIsoCode,
    ...
    bindVars = {"id": id, "geoNameId": geoNameId, "localeCode": localeCode, "continentCode": continentCode,
                ...
                "cityName": cityName, "metroCode": metroCode, "timeZone": timeZone,
                "isInEuropeanUnion": isInEuropeanUnion}
    queryResult = db.AQLQuery(aql, bindVars=bindVars, rawResults=True)
