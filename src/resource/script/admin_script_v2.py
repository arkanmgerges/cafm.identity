...
def create_permission_with_permission_contexts_for_api_endpoints():
    ...
    apiPermissionWithContexts = apiPermissionWithContextList(apiPermissionContexts, appRoutesJsonResponse, permissionList)
    hashedKeys = hashKeys(extractKeys(apiPermissionWithContexts))
    persistPermissionWithPermissionContexts(apiPermissionWithContexts=apiPermissionWithContexts, hashedKeys=hashedKeys, token=token)
    click.echo(click.style(f'Done creating permission with permission contexts and linking them', fg='green'))
    # Logout
    click.echo(click.style(f'Logout', fg='green'))
    requests.post(baseURL + '/v1/identity/auth/logout', headers=dict(Authorization='Bearer ' + token), json=dict(token=token))


def persistPermissionWithPermissionContexts(apiPermissionWithContexts, hashedKeys, token):
            ...
            if hashedKey is not None:
                try:
                    ...
                    for permissionDataItem in permissionsDataItem:
                        ...
                        assignPermissionToPermissionContext(permissionId=permId, permissionContextId=permContextId, token=token)
                except Exception as e:
                    click.echo(click.style(f'{e}', fg='red'))

def assignPermissionToPermissionContext(permissionId, permissionContextId, token):
    click.echo(click.style(f'\t---> assign permission ({permissionId}) to permission context ({permissionContextId})', fg='yellow'))
    resp = requests.post(baseURL + '/v1/identity/assignments/permission_to_permission_context', headers=dict(Authorization='Bearer ' + token),
                         json=dict({'permission_id': permissionId, 'permission_context_id': permissionContextId}))
    resp.raise_for_status()
    return checkForResult(token, resp.json()['request_id'],)


def persistPermissionContext(data, token):
    click.echo(click.style(f'\t---> persist permission context {data}', fg='yellow'))
    resp = requests.post(baseURL + '/v1/identity/permission_contexts', headers=dict(Authorization='Bearer ' + token), json=dict(data))
    resp.raise_for_status()
    return checkForResult(token, resp.json()['request_id'], checkForID=True, resultIdName='permission_context_id')

def persistPermission(data, token):
    click.echo(click.style(f'\t---> persist permission {data}', fg='yellow'))
    resp = requests.post(baseURL + '/v1/identity/permissions',  headers=dict(Authorization='Bearer ' + token), json=dict(data))
    resp.raise_for_status()
...
def apiPermissionWithContextList(apiPermissionContexts, appRoutesJsonResponse,
                                 permissionList):
        ...
        for method in appRoute['methods']:
            ...
            if not found:
                apiPermissionsToBeCreated.append(
                    {'name': f'api:{method}:{appRoute["name"]}', 'allowed_actions': [method.lower()],
                     'denied_actions': []})
...
def permissionContexts(token: str) -> List[dict]:
    permissionContextsResponse = requests.get(baseURL + '/v1/identity/permission_contexts?result_size=10000', headers=dict(Authorization='Bearer ' + token),)
...
def permissions(token: str) -> List[dict]:
    permissionsResponse = requests.get(baseURL + '/v1/identity/permissions?result_size=10000',
