...
def test_create_permissionContext_with_semantic_constructor():
    ...
    id = str(uuid4())
    # Act
    permissionContext = PermissionContext.createFrom(id=id, type=PermissionContextConstant.RESOURCE_INSTANCE.value)
    # Assert
    assert isinstance(permissionContext, PermissionContext)
...
def test_that_two_objects_with_different_attributes_are_not_equal():
    # Act
    object1 = PermissionContext.createFrom('1234', type=PermissionContextConstant.RESOURCE_INSTANCE.value)
    object2 = PermissionContext.createFrom('1234', type=PermissionContextConstant.RESOURCE_TYPE.value)
    # Assert
    assert object1 != object2
